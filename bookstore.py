...
class Database:
    ...
    def __init__(self):
        ...
        self.cur.execute("CREATE TABLE IF NOT EXISTS book (id INTEGER PRIMARY KEY, title text, price text, pcs integer, booknumber integer)")
        self.conn.commit()
        


    def insert(self,title, price, pcs, booknumber):
        self.cur.execute("INSERT INTO book VALUES (NULL,?,?,?,?)",(title, price, pcs, booknumber))
        self.conn.commit()
    ...
    def search(self,title="", price="", pcs="", booknumber=""):
        self.cur.execute("select * from book where  title =? OR price=? OR pcs=? OR booknumber =?",(title, price, pcs, booknumber))
        rows = self.cur.fetchall()
    ...
    def update(self,id="",title="", price="", pcs="", booknumber=""):
        self.cur.execute("UPDATE book SET title=?, price=?, pcs=?, booknumber=? WHERE id = ?", (title, price, pcs, booknumber,id))
        self.conn.commit()
...
def search_command():
    list1.delete(0,END)
    for row in database.search(title_text.get(),price_text.get(),pcs_text.get(),booknumber_text.get()):
        list1.insert(END, row)

def add_command():
    database.insert(title_text.get(),price_text.get(),pcs_text.get(),booknumber_text.get())
    list1.delete(0,END)
    list1.insert(END,(title_text.get(),price_text.get(),pcs_text.get(),booknumber_text.get()))
...
def update_command():
    database.update(selected_tuple[0],title_text.get(),price_text.get(),pcs_text.get(),booknumber_text.get())
    view_command()
